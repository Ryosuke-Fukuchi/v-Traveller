...
<div class="container">
  ...
  <div class="mb-5 text-center">
    <h4><a class="text-dark" href="/album/{{ $post->user_id }}/{{ $album->id }}"><i class="far fa-images pr-1"></i>{{$album->title}}</a></h4>
  </div>

  <div class="row mb-5">
    <div class=" offset-1 col-10 d-flex">
      @if($before)
      <a class="text-dark mr-auto" href="/p/album/{{ $album->id }}/{{ $before->id }}"><i class="fas fa-lg fa-angle-double-left"></i>次の投稿</a>
      @endif
      @if($after)
      <a class="text-dark ml-auto" href="/p/album/{{ $album->id }}/{{ $after->id }}">前の投稿<i class="fas fa-lg fa-angle-double-right"></i></a>
      @endif
    </div>
  </div>
 ...
 <div class="row">
   ...
   <div class="wrap col-4 bg-white">
     ...
     @can('delete', $post)
     <button type="button" class="btn btn-primary delete" data-toggle="modal" data-target="#modal1">
  ...
  <div class="modal-dialog modal-dialog-centered" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h5 class="modal-title" id="label1">「{{ $album->title }}」から写真を外す</h5>
        <button type="button" class="close" data-dismiss="modal" aria-label="Close">
          <span aria-hidden="true">&times;</span>
        </button>
      </div>
      <form action="/remove/{{ $post->id }}/{{ $album->id }}" method="post">
      <div class="modal-body">
        @csrf
        <p>(写真そのものは削除されません)</p>

        <div class="text-right">
          <button class="btn btn-primary">外す</button>
        </div>
      </div>
      </form>
    </div>
